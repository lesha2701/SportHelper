...
export async function createMetric(token: string, userId: string, input: MetricInput): Promise<Metric> {
  const dto = await apiRequest<MetricDto>(`/api/players/${userId}/metrics`, { method: "POST", successMessage: "Метрика добавлена", token, body: input });
  return mapMetricDto(dto);
}

export async function updateMetric(token: string, metricId: string, input: MetricInput): Promise<Metric> {
  const dto = await apiRequest<MetricDto>(`/api/metrics/${metricId}`, { method: "PUT", successMessage: "Метрика сохранена", token, body: input });
  return mapMetricDto(dto);
}

export async function deleteMetric(token: string, metricId: string): Promise<void> {
  await apiRequest(`/api/metrics/${metricId}`, { method: "DELETE", successMessage: "Метрика удалена", token });
}
